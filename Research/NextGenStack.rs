...
//
// BUILD TARGETS:
//   cargo build --release                          → libdvsm_core.so / .dll / .dylib
//   cargo build --release --target aarch64-...     → ARM64 (mobile/console/embedded)
//   cargo build --release --features fixed_point   → Q16.16 deterministic (deep-space)
//   cargo build --release --features no_std_alloc  → bare-metal (use static instance)
//
// LINKING:
//   C/C++:    link against libdvsm_core + include dvsm.h
//   Unity:    drop .dll/.so into Plugins/, P/Invoke dvsm_step
//   UE5:      load via FPlatformProcess::GetDllHandle, bind 5 functions
//   Python:   ctypes.cdll.LoadLibrary("libdvsm_core.so")
//   WASM:     cargo build --target wasm32-unknown-unknown (no_std only)
//
// RUNTIME CONTRACT:
//   1. Call dvsm_init(n, r) once → get handle
//   2. Call dvsm_step(handle, input_ptr, len, &out) per frame
//   3. Read out.ghost / out.stress / out.energy for decisions
//   4. Call dvsm_free(handle) on shutdown
//   Never call step from multiple threads on same handle.
//
// COMPILER FLAGS (recommended):
//   RUSTFLAGS="-C target-cpu=native -C llvm-args=-force-vector-width=16"
//
// ── CONVENIENCE WRAPPERS (feature-gated, non-ABI-stable) ────

/// Simplified step for SDK consumers who accept by-value return.
/// NOT ABI-stable across architectures (struct return convention varies).
/// Use dvsm_step() with output pointer for portable C interop.
#[cfg(feature = "sdk")]
#[no_mangle]
pub unsafe extern "C" fn dvsm_step_simple(
    core: *mut DvsmCore, input: *const f32, len: u32,
) -> BinaryFrame {
    let c = match core.as_mut() { Some(c) => c, None => return BinaryFrame::default() };
    let n = (c.params.n as u32).min(len) as usize;
    if input.is_null() || n == 0 { return BinaryFrame::default(); }
    c.step(core::slice::from_raw_parts(input, n))
}

/// Query all scalars in one call (avoids 5 separate FFI round-trips).
#[cfg(feature = "sdk")]
#[repr(C)]
#[derive(Copy, Clone, Default)]
pub struct DvsmQuickRead {
    pub stress: f32,
    pub energy: f32,
    pub drift: f32,
    pub ghost: u8,
...
#[cfg(feature = "sdk")]
#[no_mangle]
pub unsafe extern "C" fn dvsm_quick_read(core: *const DvsmCore) -> DvsmQuickRead {
    let c = match core.as_ref() { Some(c) => c, None => return DvsmQuickRead::default() };
    let r = c.params.r;
    DvsmQuickRead {
        stress: norm_safe_arr(&c.s, r) / norm_safe_arr(&c.z, r),
        energy: norm_safe_arr(&c.z, r),
        drift: stiefel_drift(&c.w, N, r),
        ghost: classify_ghost(
            norm_safe_arr(&c.s, r) / norm_safe_arr(&c.z, r),
            0.0, stiefel_drift(&c.w, N, r), 0.0,
            norm_safe_arr(&c.omega, r) / norm_safe_arr(&c.z, r),
            false, false,
        ) as u8,
        alive: c.alive,
    }
...
/// Set custom κ from physics (e.g., waveguide coupling integrals).
/// Caller must ensure: kappa[i*RMAX+j] == -kappa[j*RMAX+i].
/// Length must be >= r*r. No runtime symmetry check (O(r²) cost).
#[no_mangle]
pub unsafe extern "C" fn dvsm_set_kappa(
    core: *mut DvsmCore, kappa: *const f32, len: u32,
) -> i32 {
    let c = match core.as_mut() { Some(c) => c, None => return -1 };
    let r = c.params.r;
    if kappa.is_null() || (len as usize) < r * r { return -2; }
    let src = core::slice::from_raw_parts(kappa, r * r);
    let mut k = 0;
    while k < r { let mut j = 0;
        while j < r { c.kappa[k * RMAX + j] = src[k * r + j]; j += 1; }
        k += 1;
    }
    0
}

/// Get state hash for cross-platform parity verification.
#[no_mangle]
pub unsafe extern "C" fn dvsm_hash(core: *const DvsmCore) -> u64 {
    let c = match core.as_ref() { Some(c) => c, None => return 0 };
    let zb = core::slice::from_raw_parts(c.z.as_ptr() as *const u8, c.params.r * 4);
    let sb = core::slice::from_raw_parts(c.s.as_ptr() as *const u8, c.params.r * 4);
    fnv1a(zb) ^ fnv1a(sb)
}

// ── BinaryFrame default (needed for error returns) ──────────

impl Default for BinaryFrame {
    fn default() -> Self {
        Self {
            frame: 0, energy: 0.0, novelty: 0.0, stress: 0.0,
            stiffness: 0.0, omega_norm: 0.0, entropy: 0.0, drift: 0.0,
            resonance_peak: 0.0, ghost: Ghost::Vacuum as u8,
            contained: 1, emitted: 0, _pad: 0,
        }
    }
}

// ── FNV1a (needed by dvsm_hash, duplicated here for one-file build) ──
