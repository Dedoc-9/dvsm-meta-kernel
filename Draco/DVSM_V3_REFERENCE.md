...
//
//   Energy identity (antisymmetric κ, with backreaction):
//     d‖Z‖²/dt = −2λ‖Z‖² − 2α(‖Z‖²−E_target)‖Z‖²
//     At equilibrium: ‖Z‖² ≈ E_target, d‖Z‖²/dt ≈ −2λE_target
//
// DESIGN NOTES (ghost awareness):
//   - "Ghost" = a basis vector that collapses to near-zero but should
...
// ---------------------------------------------------------------------------
// 2.  WATTAGE PROFILE  (hardware-adaptive; drives λ, α, dt, frame budget)
// ---------------------------------------------------------------------------
...
pub struct WattageProfile {
    ...
    pub dt:           f32,
    /// Dissipation coefficient λ. Higher = faster norm decay (but backreaction fights).
    pub lambda:       f32,
...
pub struct DVSMState {
    ...
    /// Stored as row-major; κ_{kj} accessed via kappa_get()
...
impl DVSMSupervisor {
    ...
    pub fn apply_telemetry(
        ...
        // Patch dissipation and backreaction
